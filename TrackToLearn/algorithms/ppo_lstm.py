...
from TrackToLearn.algorithms.lstm_rl import LSTMRLAlgorithm
from TrackToLearn.algorithms.shared.onpolicy import LSTMActorCritic
from TrackToLearn.algorithms.shared.lstm_replay import LSTMReplayBuffer
...
class PPOLSTM(LSTMRLAlgorithm):
    ...
    def _episode(
        ...
        while not np.all(done):
            ...
            action, new_h, new_c = self.agent.select_action(
                state, h, c, stochastic=True)

            self.t += action.shape[0]

            v, prob, _, mu, std, *_ = self.agent.get_evaluation(
                state, action, h, c)

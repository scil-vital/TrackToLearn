...
if [[ "$OSTYPE" == "darwin"* ]]; then
    echo "Installing PyTorch 2.20.0"
    pip install torch==2.20.0 torchvision==0.17.0 torchaudio==2.20.0 --quiet
else
    # Install pytorch
    echo "Installing PyTorch 2.20.0+${CUDA_VERSION}"
    # Install PyTorch with CUDA support
    pip install torch==2.20.0+${CUDA_VERSION} torchvision==0.17.0+${CUDA_VERSION} torchaudio==2.20.0 --extra-index-url https://download.pytorch.org/whl/${CUDA_VERSION} --quiet
fi
